...
bool BBox::intersect(const Ray& r, double& t0, double& t1) const {
  ...
  // if ray originates within the bbox
//  if (r.o.x >= min.x && r.o.x <= max.x && r.o.y >= min.y && r.o.y <= max.y && r.o.z >= min.z && r.o.z <= max.z) return false;
...
//  if (t_min > t_max || t_min < 0 || t_max < 0 || (t_min < r.min_t && t_max < r.min_t) || (t_min > r.max_t && t_max > r.max_t)) return false;
//  if (t_max < t1) t1 = t_max;

  if ((t_min < r.min_t && t_max < r.min_t) ||  (t_min > r.max_t && t_max > r.max_t)) return false;
  if (t_min <= t_max) return true;
